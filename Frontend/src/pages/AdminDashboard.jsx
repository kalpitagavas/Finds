...
const AdminDashboard = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "customerReviews") {
      ...
      const reviews = value.split("\n").map((review) => {
        return { comment: review, rating: 5 }; // Set default rating; change as needed
      });
  ...
  const handleFileChange = (e, type) => {
    ...
    setNewProduct((prev) => ({
      ...prev,
      [type]: [...prev[type], ...files], // Add files to the corresponding array
    }));
  };
  ...
  return (
    <div
      className={`flex min-h-screen transition-all duration-500 ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-800"}`}
    >
      {/* Sidebar */}
      <aside className="w-64 p-6 bg-gray-800 text-white">
        ...
        <button
          ...
          className="mt-12 p-3 bg-gray-600 rounded-lg text-white w-full"
        >
          Toggle Mode
        </button>
      </aside>
        ...
        {products.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map((product) => (
              <div
                key={product._id}
                className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 transition transform hover:-translate-y-1 hover:shadow-xl"
              >
                ...
                {product.buyLink && (
                  <a
                    ...
                    className="text-blue-500 dark:text-blue-400 hover:underline mb-4 block"
                  >
                    Buy Now
                  </a>
                )}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="relative bg-white dark:bg-gray-800 p-6 rounded-xl w-11/12 md:w-1/2 max-h-[80vh] overflow-y-auto shadow-xl transform transition-all duration-300">
            ...
            <form
              ...
              className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto max-h-[70vh]"
            >
              {/* Product Name */}
              <div>

...
const VlogCard = () => {
  ...
  const [expandedReviews, setExpandedReviews] = useState(null);  // Track single product's reviews
  ...
  const toggleReviews = (productId) => {
    // Toggle the review section for the clicked product only
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 pb-12">
        {products
          ...
          .map((product) => (
            <div
              key={product._id}
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg w-full sm:w-64 p-4 relative transition-shadow hover:shadow-2xl"
            >
              {/* Image Container */}
              <div
              ...
              >
                <img
                  ...
                  alt={product.name}
                  className="w-full h-48 object-cover rounded-xl transition-transform"
                />
              ...
              <p className="text-gray-600 dark:text-gray-300 text-xs mt-1">{product.description}</p>
              ...
              {/* Read More Button to Expand Reviews & Write Review */}
              <button
                onClick={() => toggleReviews(product._id)}
                className="mt-3 w-full text-blue-600 font-medium py-1 rounded-lg transition-all text-sm"
              >
                {expandedReviews === product._id ? "Read Less" : "Read More"}
              </button>

              {/* Hidden Customer Reviews & Write Review Section */}
              {expandedReviews === product._id && (
                <div className="mt-4 bg-white dark:bg-gray-900 bg-opacity-70 p-3 rounded-lg shadow-md">
                  ...
                  <div className="max-h-24 overflow-y-auto text-xs space-y-1 mt-2">
                  ...
                  <button
                    ...
                    className="mt-3 w-full bg-gray-700 text-white py-1 rounded-lg hover:bg-gray-600 shadow-md transition-all text-sm"
                  >
                    Write a Review
                  </button>
